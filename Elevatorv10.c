...
*/
	void safetyPro(){
		if(SensorValue(button1) == 1 || SensorValue(button2) == 1 || SensorValue(button3) == 1 || SensorValue(limitSwitch2) == 1 || SensorValue(limitSwitch3) == 1 || SensorValue(limitSwitch1) == 1 || SensorValue(limitSwitchFloor1) == 1){
			clearTimer(T1);
		}else{
			if(SensorValue(limitSwitchFloor1) == 0 && time1(T1) >= SAFETYPROTIME){
				goTo = 1;
			}
		}
	}
