...
//Functions:
	/* Older code for lEDConfiguration
void lEDConfiguration(int lED1pos, int lED2pos, int lED3pos){
	if(LED1pos > 1 || LED1pos < 0 || LED2pos > 1 || LED2pos < 0 || LED3pos > 1 || LED3pos < 0){
			lEDSpam();
		}else{
			LED1 = lED1pos;			//this is not correct api, it's meant to set LED1 to power x
			LED2 = lED2pos;			//ditito
			LED3 = lED3pos;
			}
}

void lEDSpam(){						//Turn off motors and other things.
	while(true){
		lEDConfiguration(1,1,1);
		wait(.1);					//This may not have the correct capitioaliztion, it hsould be 100 milisecounds
...
void lEDConfiguration(int lED1pos, int lED2pos, int lED3pos){
	if(LED1pos > 1 || LED1pos < 0 || LED2pos > 1 || LED2pos < 0 || LED3pos > 1 || LED3pos < 0){
		...
		//lED1pos ? turnLEDOn(lED1) : turnLEDOff(lED1);
		//lED2pos ? turnLEDOn(lED2) : turnLEDOff(lED2);
		//lED3pos ? turnLEDOn(lED3) : turnLEDOff(lED3);
...
void lEDConfiguration(int lED1pos, int lED2pos, int lED3pos){
	void lEDSpam(){						//Turn off motors and other things.			while(true){
		lEDConfiguration(1,1,1);
		wait(.1);					//This may not have the correct capitioaliztion, it hsould be 100 milisecounds
		lEDConfiguration(0,0,0);
		wait(.1);					//ditio
